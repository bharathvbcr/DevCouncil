...
def main():
 ...
 extra=callers('gortex',read('gortex-rust-callers-include-name-only'),'db_size_gate_bytes');assert extra==EXPECTED['db_size_gate_bytes']
 supplement['gortex_include_name_only']={'rust_callers':sorted(extra),'combined_matched_pairs':5,'note':'One supplemental run; default-query measurements and four-pair score are preserved.'}
 assert json.loads(read('gitnexus-repeat-stale-query'))['symbol']['name']=='devmap_competition_revision_3'
 assert json.loads(read('gitnexus-after-force-query'))['error']=="Symbol 'devmap_competition_revision_3' not found"
 supplement['gitnexus_force_recovery']={'repeated_normal_refresh_stale':True,'force_clears_deleted_symbol':True}
