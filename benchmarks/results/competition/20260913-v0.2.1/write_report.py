...
NAMES = dict(devmap='DevMap 0.2.1', codegraph='CodeGraph', cbm='codebase-memory-mcp',
...
lines = ['# DevMap v0.2.1 competitor benchmark — 2026-09-13 UTC', '',
...
for stage, target in [('cold',None), ('warm',None), ('edit',None)] + [(s,t) for s in ['search','callers'] for t in TARGETS]:
    groups = [next(x for x in control['groups'] if (x['version'],x['stage'],x['target']) == (version,stage,target)) for version in ['0.2.0','0.2.1']]
...
lines += ['', 'Positive timing deltas mean slower observed medians; negative deltas mean faster. The old binary’s exact dirty source patch was not captured. Shared OS/compiler/package caches were not flushed, load varies, and absolute clone paths changed. These are additional limits on causal interpretation.', '',
...
lines += ['', '| Native surface | Reported file measure | Symbols/nodes | Edges |', '|---|---|---:|---:|',
    ...
    f'DevMap reports {DM["resolution_rate"]["unresolved_sites"]:,} unresolved sites, of which {DM["resolution_rate"]["explained_sites"]:,} are classified as explained, leaving {remaining:,}. The old run left 91,784 despite the same 10,193-symbol/33,789-edge totals; this classification difference has not been validated as a change in actual caller accuracy. The final [status](devmap-final-status.stdout) reports {STATUS["coverage_gaps"]["import_blind"]["total"]} SQL import-extractor gaps and {STATUS["coverage_gaps"]["pattern_recovered"]["total"]} pattern-recovered file. Coverage warnings are retained even where selected queries pass.', '',
