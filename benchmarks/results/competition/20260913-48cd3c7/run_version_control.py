"""Alternate the two preserved 0.2.1 builds after the competitor campaign, on one corpus.

The previous report measured build 506a617; the installed executable has since moved
to build 48cd3c7. Both are version 0.2.1, so the builds are keyed by build id.
"""
import hashlib
import json
...
VERSIONS = {'506a617': OLD['devmap_binary'], '48cd3c7': BINARY}
PROVENANCE = {'506a617': OLD, '48cd3c7': CURRENT}
...
def main():
    for version, provenance in PROVENANCE.items():
        assert hashlib.sha256(Path(VERSIONS[version]).read_bytes()).hexdigest() == provenance['devmap_sha256'], version
    assert PROVENANCE['506a617']['devmap_sha256'] != PROVENANCE['48cd3c7']['devmap_sha256']
    ...
    (OUT / 'version-control.json').write_text(json.dumps({
        'protocol': 'Same corpus, independent stores, alternating build order; no simultaneous timing processes. '
                    'Supplemental to the competitor samples. Both executables are version 0.2.1 and differ by 49 commits.',
        'timing_samples': len(rows), 'versions': VERSIONS,
        'builds': {version: {'build': PROVENANCE[version]['devmap_build']['id'],
                             'source_commit': PROVENANCE[version]['devmap_source_commit'],
                             'sha256': PROVENANCE[version]['devmap_sha256']} for version in VERSIONS},
        'groups': groups, 'answers': answers,
