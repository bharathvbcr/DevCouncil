...
``python -m pytest`` puts the working directory there itself, so it collected
4462 tests.  The bare ``pytest`` console script does not, and aborted with
``ModuleNotFoundError: No module named 'tests'`` on those eighteen — 4178
collected, 18 errors.  Same tree, same command-line arguments, different suite,
decided by which of the two forms you typed.  It is a collection *error* rather
than a failure, so a run with ``--continue-on-collection-errors`` reported the
other 4178 green.

``uv run pytest`` behaves as the bare console script, not as ``-m`` — measured,
not assumed — and that is what three of the four CI workflows type:
``npm-publish.yml``, ``codeintel-platform.yml`` and ``devcouncil-evidence.yml``
all run it, while ``ci.yml`` runs ``coverage run -m pytest``.  So CI was
collecting two different suites and calling both the test run.
...
def test_both_entry_points_collect_the_same_count() -> None:
    """Not just that each collects, but that they agree.

    The two tests above prove the bare script *can* collect the module. This is
    the property that was actually violated: two invocations of the same tree
    reporting different suite sizes. It compares counts rather than exit codes,
    because a future divergence need not error — a module quietly collected
    under one form and not the other would leave both green.
    """
    console_script = Path(sys.executable).parent / "pytest"
    if not console_script.exists():  # pragma: no cover - environment dependent
        pytest.skip(f"no pytest console script beside {sys.executable}")

    # Stripped for the same reason as above: the parent's sys.path must not
    # answer the question the child is being asked.
    env = {k: v for k, v in os.environ.items() if k != "PYTHONPATH"}
    args = ["--collect-only", "-q", "-p", "no:cacheprovider", str(PACKAGE_IMPORTING_MODULE)]

    runs = {
        "console script": [str(console_script), *args],
        "python -m pytest": [sys.executable, "-m", "pytest", *args],
    }
    counts = {}
    for name, argv in runs.items():
        ...
        counts[name] = _collected(result.stdout, name)

    assert len(set(counts.values())) == 1, f"the entry points disagree: {counts}"
    assert next(iter(counts.values())) > 0, "both collected nothing, so this proves nothing"


def _collected(output: str, name: str) -> int:
    ...
    for line in reversed(output.splitlines()):
        if "collected" in line:
            for token in line.split():
                if token.isdigit():
                    return int(token)
    raise AssertionError(f"no collection count in {name} output:\n{output}")
