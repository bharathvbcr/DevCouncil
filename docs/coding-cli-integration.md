...
- **Assistive hooks** — `Stop`, `SessionStart`, `UserPromptSubmit`, `SessionEnd`, `PreCompact`, `PostCompact`, `SubagentStop`, and `Notification` are wired into `.claude/settings.local.json`. `SessionStart`/`UserPromptSubmit` inject a live DevCouncil status snapshot as context. After Claude Code context compaction, `PreCompact` atomically writes `.devcouncil/state/compact_snapshot.json` (phase, active task, blocking gaps, last stop-gate event, optional last assistant sentence); a subsequent `SessionStart` with `source: "compact"` injects a slim recovery briefing from that snapshot instead of the full status. `UserPromptSubmit` skips the status line for ~60 seconds after compaction to avoid duplicate context. `PostCompact` is trace-only. These never block a tool call.
