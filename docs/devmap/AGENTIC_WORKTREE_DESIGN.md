...
Agents should pass an absolute repository root through the GitPulse MCP adapter. Omitted CLI repository roots resolve the nearest Git worktree root, including from nested directories, consistently across builds, queries, discovery, and rendering. Explicit paths retain their scope, including an explicit `.`. An explicit `--db` can intentionally inspect a snapshot from another directory; relative database overrides are invocation-relative. Non-Git trees still use the supplied/current root. Generated state is not copied by Git: resolve it with `devmap paths --json`, inspect `devmap status --json`, and bootstrap or restore exported maps with `devmap build --manifest`. Read-only discovery never initializes a store or inherits a sibling worktree's database.
