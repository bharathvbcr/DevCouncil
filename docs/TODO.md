...
| **TASK-P7-9** | Rewrite packaged `devcouncil.md` / hero-loop / verification skills and policy `NoTaskAllowedCommands` to live Go MCP, Manvi, and DevMap commands. `dev checkout` / `dev status` / slash `/devcouncil:*` are unknown (exit 2). **Also cut the enforcement claim** (2026-09-13): the packaged verification skill still tells teams `verification.diff_coverage.enforce: true` and `enforce_coverage_on_hard` promote a gap to blocking. Nothing in this tree reads either — since TASK-P7-1 `verify.Run` spawns `dcverify`, but the coverage gap is `Blocking: false` by construction in `verify/rigor.go` and difficulty is never sent to the binary. The harness flags that mirrored them (`verify.diff_coverage.enforce`, `verify.rigor.enabled`) were removed from the catalogue and `docs/hero-loop.md` corrected, leaving the skill body as the last place the claim survives — in **five** checked-in copies, not two: `backend/go_orchestrator/devcouncil/skills/library/devcouncil-verification.md:64-65` (the source the others are generated from) plus `.claude/`, `.agents/`, `.cursor/` `skills/devcouncil-verification/SKILL.md:61-62` and `.devcouncil/claude-plugin/devcouncil/skills/devcouncil-verification/SKILL.md:52-53`. Fix the library copy and re-scaffold; editing one host copy leaves four. | `GAP-P7-SKILLS-PYTHON-TOOLS`, `GAP-P7-POLICY-CLI-DRIFT` |
