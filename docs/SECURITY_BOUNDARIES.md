...
| `dcjsoncheck` | Manifest stdin capped at 1 MiB; per-invocation stdout/stderr capture capped at 8 MiB, and exceeding it fails that invocation rather than truncating its evidence |
...
whole-process RSS ceiling. `dcjsoncheck` bounds what it reads and keeps but has
no wall-clock deadline of its own: an invocation that never exits is bounded by
whatever runs the harness. Its manifest and its commands are the operator's,
