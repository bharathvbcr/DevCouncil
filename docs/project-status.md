...
| **Agent Integrations** | `devcouncil integrate` | **Preview** | Cursor: `.cursor/mcp.json` + rule. Claude: `.mcp.json` only. Codex: comment-only toml. Antigravity / OpenCode / Warp / Aider / Gemini: stub receipt. `--write-gate` is refused before writes. See TASK-P7-8. |
