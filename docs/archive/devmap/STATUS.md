...
> **Archived 2026-09-11** to [docs/archive/devmap/STATUS.md](STATUS.md). Pre-Phase-7 kernel ledger. The Python engine (`src/devcouncil/`) is gone. Live: [code-graph.md](../../code-graph.md), [TODO.md](../../TODO.md), [PHASE7_LONG_TAIL.md](../../PHASE7_LONG_TAIL.md).

**Rust hardening audit, 2026-09-10:** import containment, notebook source/cell integrity, query parsing, transactional ID boundaries, watcher shutdown, MCP deadlines/cancellation, release panic recovery and clone ranking are repaired. The final workspace passed 2,543 tests plus 667 parser-free tests; 128-worktree and eight-larger-graph workloads, the 40-cycle storm, and the longer growth qualification passed. The retained 100/500-cycle WAL warm-up failures, exact source identities, API changes and native-platform limits are in [RUST_AUDIT_2026-09-10.md](RUST_AUDIT_2026-09-10.md). Canonical integration is pending concurrent MCP-root changes; the qualified fixes remain in the isolated audit worktree. Installed clients and production stores were not changed.
...
**Active work:** Phase 7 host-assets Python deletion closed 2026-09-10; kernel PDG/adjunct polish remains
**Verified scope:** local Rust kernel, Go `devcouncil` binary (mcp/integrate/skills/verify), thin Python launcher
...
### Phase 7 — adjuncts + host-assets Python deletion

- **Host-assets Phase 7 closed 2026-09-10:** Python orchestration packages deleted;
  thin `dev` launcher only; decisions in [docs/PHASE7_LONG_TAIL.md](../docs/PHASE7_LONG_TAIL.md).
  LSP **cut** (DIVERGENCES). FAISS/GEPA/campaign/live/ui/telemetry/reporting/wiki **retired**.
- The PDG kernel continues to consume an explicit structured statement tree (CFG contract work).
- Open (kernel only): grammar extraction into structured PDG input, durable storage/query
  integration, security sink classification — not blockers for the host-assets cut.
...
`edge_rows(source_symbol, valid_from, valid_to)`, and whether SQLite will use it under
the view's join is unmeasured. That is the next question, not a rebase of this one.

What v18 did change here, and had to: the read order's last tie-break. It was the
emission `ordinal`, which v18 cannot supply — a row carried across generations keeps the
`edge_id` of the generation that first inserted it, so an incremental generation and a
cold one would order the same edges differently. V replaced it with `resolution`, and
