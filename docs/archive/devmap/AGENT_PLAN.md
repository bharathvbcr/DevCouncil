...
| W0.3-2 | Import extraction for the C family and the JVM/CLR languages | DEVMAP_HARDENING.md W0.3 | **open** — W0.3 shipped move 1 ("stop the bleeding": an import-blind file is excluded from `unwired_candidates` and counted) and named move 2 as the follow-up. `#include` still has no handler anywhere in the extractor and 23 of 35 languages declare no `IMPORTS` capability, so `unwired_candidates` is permanently empty for Java, C, C++, C#, Ruby, Swift, Kotlin, Scala, Dart, PHP and Objective-C. On this repository that is 71 files; in a Java or C++ shop it is the whole tree. `unwired_is_not_import_blind.rs` keeps the pre-gate assertion deliberately, so it goes red when this lands and the failure is the news |
| SC29 | Unexplained SC26 per-file memory increase | STATUS.md | open |
| SC2/B3 | Write amplification + genuinely incremental resolve | PLAN.md §7.5 | **half closed 2026-09-06 (schema v17), half open with a measured size and a named design.** §7.5 framed this as row count; measured, the cost was bytes and it was in one table: `generation_files` was **54% of a 302.8 MB store** on 1,531 rows per generation, each carrying a serialized `Extraction` averaging 53.7 KB, and **1,530 of its 3,062 rows were byte-identical duplicates**. v17 stores one payload per (file, content, language, grammar, analyzer) and keeps `generation_files` as a view over a 16-byte membership row. A/B on one fixture: payload bytes added by a one-file edit went from **100% to 1%**; on this repository the store after that edit is 302.8 -> 217.6 MB and `extraction_json` 164.5 -> 82.4 MB. **Still open:** `generation_edges` (96,525 rows, 50.2% duplicated across generations) and `generation_unresolved` (89,537), about **67 MB** of the remaining per-generation growth. These need the **validity-range** design, not the content-addressing v17 used, and the reason is measurable rather than stylistic: both carry composite indexes spanning the generation *and* a row column — `idx_generation_edges_source(generation_id, source_file_id)`, `_target`, `idx_generation_unresolved_callee(generation_id, callee_name)`, `_class` — so splitting the row from its membership puts the two halves of every one of those indexes in different tables. A row that carries its own `[valid_from, valid_to)` keeps them intact. The open decision is what that costs on the read side: `generation_id = ?` becomes a range predicate, so the latest generation wants a partial index on `valid_to IS NULL` and historical reads fall back to a scan |
