...
| SC2/B3 | Write amplification + genuinely incremental resolve | PLAN.md §7.5 | **decision** (#2) — **re-measured 2026-09-06 and this row's premise was stale.** On a 1,060-file / 58,651-edge corpus a one-file edit costs extract 0.565 s, resolve 0.340 s (**15%**), analyze 0.109 s (5%), persist 1.221 s (**55%**). STATUS.md's "resolve and analyze are 54% of a rebuild" was measured on a different corpus and does not hold here, so a genuinely incremental *resolve* is not the lever it was recorded as. The narrowed write and a `--full` write cost about the same (~1.2 s each) — write amplification measured rather than argued, and still the whole of the remaining cost. The decision is unchanged and unstarted: it is the validity-range schema redesign |
