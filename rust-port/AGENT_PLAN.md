...
| ID | Item | State |
|---|---|---|
| `DOC-1` | `PLAN.html` is a 2026-08-12 snapshot with different structure (8 sections vs PLAN.md's 11) and no generator. Its own `.md` described it as "same content, styled", which was already false before this pass | **open** — either write a generator or retire the file; PLAN.md and README.md now describe it honestly in the meantime |
| `DOC-2` | `AUDIT.html` has the same shape of problem — a rendered snapshot with no `.md` source in the tree and no generator | **open**, unaudited |
...
| `DOC-4` | `CONSUMERS.md` does not mention the `devmap_engine.py` seam changes or the Python embedding replacement (K7) | **open** |
