...
| `DOC-2` | `AUDIT.html` has the same shape of problem, and a worse one: there is **no `AUDIT.md` in the tree**, so it cannot be regenerated — the rendered artifact is the only copy and its source is gone | **open** — not a generator problem. Either recover the source, extract markdown back out of the HTML, or retire the file. Retiring needs proof it is unreferenced; README.md links it today |
| `DOC-3` | `rust-port/CLAUDE.md` and `AGENTS.md` are generated by `dev map` and currently degenerate (`crates: model, model, model`). Regenerating needs a `dev map` run, which was deliberately not done while the tree was mid-edit | **open** — run `dev map` once the tree is quiet |
...
`DOC-3` is blocked rather than deferred: `dev map` writes a shared index, and the tree has had
a second session writing Rust source continuously through this audit (most recently 17 seconds
before the last check). Running it now would index a half-built state into a store both sessions
read. Run it once the tree is quiet.
