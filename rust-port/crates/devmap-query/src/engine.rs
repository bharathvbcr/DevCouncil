...
impl<'a> StoreQueryEngine<'a> {
    ...
    pub fn search(&self, req: Request<String>) -> anyhow::Result<Response<SymbolHit>> {
        ...
        for row in rows {
            let score = name_match_score(&row, &query);
...
/// Score a stored symbol against a lower-cased query.
///
/// One owner for the ranking so `search` and `explore` order the same corpus the
/// same way; two copies would let the same query return a different "best match"
/// depending on which command asked.
fn name_match_score(row: &StoredSymbol, lowered_query: &str) -> f32 {
    let name = row.name.to_lowercase();
    let qualified = row.qualified_name.to_lowercase();
    if name == lowered_query || qualified == lowered_query {
        1.0
