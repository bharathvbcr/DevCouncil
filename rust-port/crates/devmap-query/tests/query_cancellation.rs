...
fn a_cancelled_corpus_scan_stops_partway_through() {
    let texts = corpus(60_000);

    let started = Instant::now();
    SemanticIndex::build(&texts, &Cancel::new()).expect("an uncancelled build must finish");
    let baseline = started.elapsed();
    assert!(
        baseline >= Duration::from_millis(300),
        "the fixture completes in {baseline:?}; too fast for 'stopped early' to mean \
         anything, so this test would pass without checking anything"
    );
