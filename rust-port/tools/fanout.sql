...
--   files | fanout_edges | sum_n2 | sites | max_n | unjoined | alt_sum_n2 | alt_sites
--
-- `unjoined` must be 0 and `alt_*` must equal their primaries; a caller that
-- does not check both is reading a number it has not validated.
...
SELECT f.sf AS sf, f.ss AS ss, nm.name AS callee
  FROM _fanout f
  LEFT JOIN _nm nm ON nm.file_id = f.tf AND nm.qualified_name = f.ts;

CREATE TEMP TABLE _grp AS
SELECT sf, ss, callee, COUNT(*) AS n FROM _joined GROUP BY 1, 2, 3;
