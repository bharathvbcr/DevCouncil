...
Concretely, it builds a persistent Requirement → Task → Diff → Evidence graph. A one-line goal gets debated into typed requirements and a scoped task list by a small "council": two independent planners, two adversarial critics, a rebuttal round, an arbiter. Each task can then only touch the files it declared and run the commands it's allowed to. When the agent says it's finished, DevCouncil verifies the diff four ways: did it stay in scope, did the tests actually pass, was the *new* logic exercised, and is this real code instead of a stub. It turns each acceptance criterion into a runnable check. If the evidence isn't there, the task doesn't pass. It's *blocked*, with a specific gap. Not a vibe.
...
So DevCouncil builds one. `dev map` parses your repo with tree-sitter into a symbol-level graph: files, symbols, imports, call sites, subsystems. It figures out the important surfaces for *any* repo by ranking the import graph, with no hand-written config. Subsystems come with entry points, critical files, role buckets (runtime, policy, adapters), neighbors, and handoff paths so an agent can follow how work actually flows across the codebase instead of guessing from folder names.

The map stays honest about its own age. An ordinary edit marks it stale. A *missing* map is treated as stale too, never trusted. On hard rigor, a stale map fails closed. `dev map --watch` and post-tool hooks refresh it incrementally so you aren't babysitting regenerations after every change.

That map earns its keep in several places at once. Planning grounds itself in real files and symbols instead of hallucinated ones. Right before a task changes a file, DevCouncil hands the agent the **blast radius**: every file that imports the one being edited, so it edits in place and keeps the call sites working. Scoped prompts also get a symbol outline of the planned files and structural orientation from the map, so the agent starts oriented instead of spelunking. Write policy soft-blocks edits that jump into a foreign subsystem unless that area is a map neighbor or you widen scope on purpose.

You can walk the graph yourself. `dev graph query` finds callers and callees. `dev graph trace` shows how two symbols connect. `dev graph dead` flags dead code with confidence tiers so "extracted" findings aren't confused with noisier inferred ones. `dev graph search` and `dev graph cypher` let you ask the graph structured questions, an opt-in program-dependence graph goes deeper when you actually need data flow, and `dev graph html` serves an interactive map of the whole repo. Underneath it is real machinery, not a one-off script: a tree-sitter parse behind a sha256-keyed cache so a medium repo doesn't re-parse from scratch every run, an AST matcher (`dev ast match`) for structural search, a semantic index, and optional language-server detection. From the same foundation, `dev wiki update` builds an agent-facing wiki: one cross-linked page per subsystem, a development guide, a change log, knowledge that planning and prompts can pull in by subsystem tag.
...
## The rest of the loop around that map

The map is the eyes. The rest of DevCouncil is the hands and the conscience.

You can stay in a manual sidecar: `dev plan`, `dev prompt`, paste into your agent, `dev verify`. Or you can close the loop with `dev go` / `dev e2e` and an automated executor. When verification fails, a bounded self-repair loop writes a correction manifest and re-drives the agent until the gaps close or the budget is spent. No-progress detection stops early when the same blockers keep coming back, so you don't burn tokens on a stuck retry.

Live review sits beside that. `dev watch` can read a coding session as it happens, write critique cards, and block verify on open critical issues until you resolve them. The MCP server lets an agent drive checkout, lease-gated writes, verify, and repair from inside Cursor, Claude Code, Codex, OpenCode, and friends: same policy, same evidence, without leaving the chat. Hooks can stop unauthorized writes before they land. A local dashboard surfaces blocking gaps and recent runs. Run supervision treats each automated run as something you can timeline, diff, and revert from checkpoints when the verdict says so.

When one task at a time isn't enough, `dev campaign` runs a whole task graph as a parallel team: a director plans it, a coordinator hands work to a pool of workers, and a reviewer does QC, with automatic serialization when two agents would touch the same file, a cost budget so a run can't quietly overrun, and a push notification when it needs you. And everything an agent does stays on the record: gated file changes, verification runs, and coverage evidence are captured as provenance you can audit, secrets are redacted out of every log, and all of it lives in a local `.devcouncil/` folder that never leaves your machine.

Setup wires the boring parts: repo map, `AGENTS.md` / `CLAUDE.md` guides kept in sync with the map, applicable engineering skills scaffolded into the agent, optional CI that verifies PRs and uploads evidence. `dev doctor` sanity-checks your environment down to your chip, RAM, and whether your local context window is big enough, `dev cost` keeps a running ledger of model spend with caps that warn before a loop overruns, and a design-system gate can fail a build on styling that bypasses your design tokens. It runs against OpenRouter, Vertex, Doubleword, or fully offline on Ollama at zero cost. The point of all of that surface area is the same personality: authorize the change, ground it in the real codebase, then refuse to call it done without proof.
