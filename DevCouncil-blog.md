...
You can walk the graph yourself. `dev graph query` finds callers and callees. `dev graph trace` shows how two symbols connect. `dev graph dead` flags dead code with confidence tiers so "extracted" findings aren't confused with noisier inferred ones. `dev graph view` serves an interactive map of the whole repo. From the same foundation, `dev wiki update` builds an agent-facing wiki: one cross-linked page per subsystem, a development guide, a change log, knowledge that planning and prompts can pull in by subsystem tag.
