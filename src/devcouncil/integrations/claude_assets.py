...
def _plugin_lsp_json(root: Path) -> str | None:
    """Build the plugin's ``.lsp.json`` from DevCouncil's language-server detection.

    DevCouncil already knows which LSP servers match the repo's languages (it detects
    them for the repo map); this ships that knowledge to Claude Code so the plugin grants
    real code intelligence (go-to-definition, references, diagnostics) instead of only
    detecting servers. One server per detected language (the preferred candidate), keyed by
    language name, in Claude Code's documented schema: ``command`` + ``args`` +
    ``extensionToLanguage``. The consumer still installs the binary (Claude Code flags a
    missing one in ``/plugin`` Errors). Returns None when the repo has no supported
    languages, so the bundle simply omits the file."""
    from devcouncil.indexing.lsp import LspInspector

    inspector = LspInspector(root)
    ext_by_language: dict[str, dict[str, str]] = {}
    for ext, language in LspInspector._EXTENSIONS.items():
        ext_by_language.setdefault(language, {})[ext] = language

    servers: dict[str, dict] = {}
    for candidate in inspector.server_candidates():
        if candidate.language in servers:
            continue  # first (preferred) candidate per language wins
        command = list(candidate.command)
        entry: dict = {"command": command[0]}
        if command[1:]:
            entry["args"] = command[1:]
        entry["extensionToLanguage"] = ext_by_language.get(candidate.language, {})
        servers[candidate.language] = entry
...
_GITHUB_WORKFLOW = """\
# DevCouncil + Claude Code — event-driven task runner.
#
# Generated by `dev integrate claude-github`. This is the stable-primitive alternative to
# Claude Code's (experimental) cloud Routines: it runs DevCouncil's verify loop on pull
# requests and, on demand or on a schedule, hands the next task to headless Claude Code —
# staying inside DevCouncil's evidence/verify gate the whole way.
#
# Setup: add an ANTHROPIC_API_KEY repository secret for the autonomous (dispatch/schedule)
# runs. The pull_request job needs no secret; it only runs read-only verification.
name: DevCouncil
...
on:
  ...
  workflow_dispatch:
  schedule:
    - cron: "0 6 * * *"  # nightly task pickup (UTC); adjust or remove as needed
...
jobs:
  verify:
    # Read-only DevCouncil verification on every PR — no model calls, no secrets.
    ...
    steps:
      ...
      - name: Install DevCouncil
        run: |
          ...
          echo "$HOME/.local/bin" >> "$GITHUB_PATH"  # put the dev/devcouncil CLIs on PATH
  ...
  autonomous:
    # On demand or on schedule: let headless Claude Code pick up and implement the next task
    # through the DevCouncil MCP loop, then verify.
    if: github.event_name != 'pull_request'
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      ...
      - uses: actions/setup-node@v5
        with:
          node-version: "24"
      - name: Install DevCouncil and Claude Code
        run: |
          uv tool install devcouncil
          echo "$HOME/.local/bin" >> "$GITHUB_PATH"  # put the dev/devcouncil CLIs on PATH
          npm install -g @anthropic-ai/claude-code
      - name: Wire DevCouncil into Claude Code
        run: dev integrate claude --apply
      - name: Implement next task (headless, gated)
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
        # --dangerously-skip-permissions is appropriate here: the runner is an ephemeral
        # sandbox, and DevCouncil's own scope gate + the verify step below still constrain
        # the result. Without it, headless Claude blocks on MCP-tool permission prompts.
        run: |
          claude -p --dangerously-skip-permissions --output-format json \\
            "Use the DevCouncil MCP tools to implement the next task: call \\
             mcp__devcouncil__devcouncil_next_task, checkout a lease, make only the \\
             authorized changes, then verify and release the lease."
      - name: Verify result
        run: dev verify || true
"""


def build_github_workflow(root: Path) -> GeneratedAsset:
    """Generate ``.github/workflows/devcouncil.yml``.

    Delivers the event-driven-run value of Claude Code Routines using stable primitives
    (GitHub Actions + headless Claude Code) instead of the experimental cloud API: read-only
    DevCouncil verification on PRs, and gated autonomous task pickup on dispatch/schedule."""
    return GeneratedAsset(root / ".github" / "workflows" / "devcouncil.yml", _GITHUB_WORKFLOW)
...
def build_plugin_bundle(
    ...
    # Ship an .lsp.json (auto-discovered at plugin root) for the repo's languages, so the
    # plugin delivers live code intelligence — omitted entirely when nothing is detected.
