"""File-based agent mailbox — the Director message bus.

Faithful port of the original ``scripts/inbox_write.sh`` semantics:

* one YAML file per agent at ``.devcouncil/campaign/inbox/<agent>.yaml`` holding a
...
* every append happens under a cross-process lock (an ``mkdir`` spin-mutex, plus
  ``fcntl.flock`` where available) so concurrent Worker never corrupt a file;
...
:mod:`devcouncil.campaign.watcher`.
"""
...
class MailboxLockTimeout(MailboxError):
    """Raised when the mailbox mutex cannot be acquired within :data:`LOCK_TIMEOUT_S`."""
...
class Mailbox:
    ...
    def __init__(self, root: Path | str = Path(".")):
        self.root = Path(root)
        self.inbox_dir = self.root / ".devcouncil" / "campaign" / "inbox"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() > deadline:
                        # Best-effort stale-lock recovery: if the mutex dir is very old,
                        # assume a crashed writer left it behind and try once more.
                        try:
                            age = time.time() - self._mutex_dir.stat().st_mtime
                            if age > LOCK_TIMEOUT_S * 2:
                                self._mutex_dir.rmdir()
                                continue
                        except OSError:
                            pass
                        raise MailboxLockTimeout(
                            f"could not acquire mailbox lock within {LOCK_TIMEOUT_S}s: {self._mutex_dir}"
                        )
                    time.sleep(0.02)
    ...
    def _cap(messages: List[Message]) -> List[Message]:
        ...
        result = [m for m in messages if id(m) in keep]
        if len(result) > MAX_MESSAGES:
            # Unread dominated the cap — drop oldest messages to honor MAX_MESSAGES.
