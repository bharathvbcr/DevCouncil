...
from devcouncil.repo.ci_scaffold import WORKFLOW_RELPATH, EVIDENCE_WORKFLOW_RELPATH, detect_stacks, scaffold_ci, scaffold_evidence_ci
...
def scaffold_ci_command(
    ...
    evidence: bool = typer.Option(False, "--evidence", help="Also write .github/workflows/devcouncil-evidence.yml."),
):
    """Write a starter GitHub Actions workflow derived from the configured commands."""
    root = project_root.expanduser().resolve()
    from devcouncil.telemetry.logging_setup import set_log_dir
    set_log_dir(root)
    logger.info("dev scaffold-ci: force=%s evidence=%s", force, evidence)
    ...
    with log_stage("scaffold", project_root=root, force=force):
        ...
        if evidence:
            log_step("scaffold/2: writing evidence workflow", project_root=root, trace=True)
            evidence_target = scaffold_evidence_ci(root, force=force)
            if evidence_target is None:
                console.print(
                    f"[yellow]{EVIDENCE_WORKFLOW_RELPATH.as_posix()} already exists. "
                    f"Re-run with --force to overwrite.[/yellow]"
                )
            else:
                console.print(f"[green]Wrote {evidence_target.relative_to(root).as_posix()} (verify → evidence artifacts → optional GitHub Check/comment).[/green]")
