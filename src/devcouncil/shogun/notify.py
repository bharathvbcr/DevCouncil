"""Notifications to the Lord — a port of the original ``ntfy.sh`` push.

The Karo (and only the Karo/Shogun) may reach the Lord. When a campaign finishes
— or a task is blocked — a one-line push goes to an `ntfy <https://ntfy.sh>`_
topic so the Lord can watch the battle from a phone. A missing topic degrades to
...
class Notifier:
    ...
    ----------
    topic:
        ntfy topic. Falls back to ``$SHOGUN_NTFY_TOPIC``. Empty → disabled.
    server:
        ntfy server base URL (default the public ``https://ntfy.sh``). Falls back
        to ``$SHOGUN_NTFY_SERVER``.
    enabled:
    ...
    def __init__(
        ...
        self.topic = topic or os.environ.get("SHOGUN_NTFY_TOPIC", "")
        self.server = (server or os.environ.get("SHOGUN_NTFY_SERVER") or "https://ntfy.sh").rstrip("/")
    ...
    def notify(
        ...
        try:  # httpx is a DevCouncil dependency; import lazily to keep startup cheap.
            ...
            token = os.environ.get("SHOGUN_NTFY_TOKEN")
            if token:
...
class NullNotifier(Notifier):
    ...
    def notify(self, *args: object, **kwargs: object) -> bool:  # type: ignore[override]
