...
The Karo tags every subtask with a Bloom level and routes it: the lower three
levels (Remember / Understand / Apply) are execution and go to an **Ashigaru**;
the upper three (Analyze / Evaluate / Create) are cognition and go to the
**Gunshi**. The classifier is a deterministic keyword heuristic — intentionally
...
def route_rank(level: BloomLevel) -> Rank:
    ...
    return Rank.GUNSHI if level >= BloomLevel.ANALYZE else Rank.ASHIGARU
...
def summarize_routing(items: Iterable[str]) -> Dict[str, int]:
    ...
    counts = {Rank.ASHIGARU.value: 0, Rank.GUNSHI.value: 0}
    for item in items:
