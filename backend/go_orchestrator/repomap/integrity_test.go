...
// writeIntegrityGraph renders a graph document to a temp file.
func writeIntegrityGraph(t *testing.T, doc map[string]any) string {
	t.Helper()
...
func TestAnArtifactBehindItsIndexIsReported(t *testing.T) {
	m, err := Load(writeIntegrityGraph(t, stamped(2, "ok", nil)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAnArtifactMatchingItsIndexIsSilent(t *testing.T) {
	m, err := Load(writeIntegrityGraph(t, stamped(4, "ok", nil)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAnUnstampedArtifactIsUnverifiedNotAgreed(t *testing.T) {
	...
	delete(doc, "meta")
	m, err := Load(writeIntegrityGraph(t, doc))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAPartialAnalysisIsReported(t *testing.T) {
	m, err := Load(writeIntegrityGraph(t, stamped(4, "partial", nil)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestEdgesWithNoNodeAreReported(t *testing.T) {
	m, err := Load(writeIntegrityGraph(t, stamped(4, "ok", map[string]any{"edge_endpoints_without_node": 349})))
	if err != nil {
		t.Fatal(err)
	}
	degraded := strings.Join(m.Degraded(), " ")
