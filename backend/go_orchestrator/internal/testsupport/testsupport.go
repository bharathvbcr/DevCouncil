...
// RustWorkspace is the repository-relative directory holding the analysis
// plane's Cargo workspace.
//
// It is a named constant rather than a literal repeated at each use because it
// is the one fact in this file that changes when the tree is rearranged, and
// spelling it five times is five chances to move four of them. This port
// already spent that: the crates were `crates/` in the harness they came from
// and are `rust/` here, and every unconverted spelling failed as "no repository
// root", which names the symptom and not the cause.
//
// RepoRoot is defined as the directory containing it, so this constant also
// decides where the walk below stops.
const RustWorkspace = "rust"
...
func RepoRoot(t testing.TB) string {
	...
	for {
		if _, err := os.Stat(filepath.Join(dir, RustWorkspace, "Cargo.toml")); err == nil {
			return dir
		}
		parent := filepath.Dir(dir)
		if parent == dir {
			t.Fatalf("no repository root above %s (looking for %s/Cargo.toml)", dir, RustWorkspace)
		}
		dir = parent
	}
}
...
func cargoBin(t testing.TB, crate, binary string) string {
	...
	b.once.Do(func() {
		crates := filepath.Join(root, RustWorkspace)
