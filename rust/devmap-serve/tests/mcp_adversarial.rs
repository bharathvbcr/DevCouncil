...
/// Both transports, same question, same graph payload.
///
/// MCP additionally self-attributes (`repository`, and on status
/// `candidate_roots` / `ambiguous`) because one process is shared across
/// workspace tabs. The Unix socket is a single-store daemon and has no
/// equivalent envelope; those keys are stripped before the payloads are
/// compared so a genuine answer drift cannot hide behind them.
#[tokio::test]
async fn the_two_transports_answer_the_same_question_identically() {
    ...
    for (tool, arguments, mut ipc) in cases {
        ...
        let mut mcp_body = mcp["result"]["structuredContent"].clone();
        if let Some(object) = mcp_body.as_object_mut() {
            object.remove("repository");
            object.remove("candidate_roots");
            object.remove("ambiguous");
        }
        assert_eq!(
            mcp_body, envelope["result"],
